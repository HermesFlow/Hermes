...
public:
    ...
    Py::Object setHtml(const Py::Tuple& args);
    Py::Object registerHandler(const Py::Tuple& args);
...
void BrowserViewPy::init_type()
{
    ...
    add_varargs_method("registerHandler",&BrowserViewPy::registerHandler,"registerHandler(str)");
}
...
Py::Object BrowserViewPy::registerHandler(const Py::Tuple &args)
{
    const char* cmd;

    //Read Command
    if (!PyArg_ParseTuple(args.ptr(), "s",&cmd))
        throw Py::Exception();

    if (myWebView)
        myWebView->RegisterSlotHandler(QString::fromUtf8(cmd));
...
{
    QWebView::load(url);
    ConnectWebFrames(page()->mainFrame());
}

void WebView::ConnectWebFrames(QWebFrame* pFrame)
{
    //NULL Frame
    if (pFrame == 0)
        return;
    ...
    //Add ourself
    pFrame->addToJavaScriptWindowObject(QString::fromUtf8("webview"), this);

    //Check Frames Below
    QList<QWebFrame *> pChildFrames = pFrame->childFrames();
    const int nChildCount = pChildFrames.size();
    for (int i = 0; i < nChildCount; i++)
        ConnectWebFrames(pChildFrames[i]);
}

void WebView::RegisterSlotHandler(const QString sData)
{
    //Split Data
    const QStringList sDataList = sData.split(QString::fromUtf8(","));
    const int nSlotId = sDataList.value(0, QString::fromUtf8("0")).toInt();
    const QString sObjectName = sDataList.value(1, QString::fromUtf8(""));

    //Index
    const int nIndex = m_nSlotIdList.indexOf(nSlotId);

    //Not Found
    if (nIndex == -1)
    {
        m_nSlotIdList.push_back(nSlotId);
        m_sSlotObjectList.push_back(sObjectName);
    }
    else
        m_sSlotObjectList[nIndex] = sObjectName;
}


void WebView::process(int nSlotId, const QString sData)
{    
    //Search Slot Handler
    const int nIndex = m_nSlotIdList.indexOf(nSlotId);

    //ObjectName
    const QString sObjectName = m_sSlotObjectList.value(nIndex, QString::fromUtf8(""));

    //Didn't Find Object
    if (sObjectName.isEmpty())
        return;

    //QString sMsg = QString::fromUtf8("I Got Here\n\n\n");
    Gui::PythonConsole* pyConsole = Gui::getMainWindow()->findChild<Gui::PythonConsole*>();
    if (pyConsole)
        pyConsole->blockPrintStatement(true);

    QString sCmd;
    //Get Active Document
    sCmd.append(QString::fromUtf8("__doc__=Gui.ActiveDocument"));
    Gui::Command::runCommand(Gui::Command::Doc, sCmd.toUtf8().data());

    //Get Object
    sCmd = QString::fromUtf8("__obj__ = __doc__.getObject('");
    sCmd.append(sObjectName);
    sCmd.append(QString::fromUtf8("')"));
    Gui::Command::runCommand(Gui::Command::Doc, sCmd.toUtf8().data());

    //Process Command
    sCmd = QString::fromUtf8("__obj__.Object.Proxy.process('");
    sCmd.append(sData);
    sCmd.append(QString::fromUtf8("')\n"));
    Gui::Command::runCommand(Gui::Command::Doc, sCmd.toUtf8().data());
    ...
    //Stop Blocking Print
    if (pyConsole)
        pyConsole->blockPrintStatement(false);

    //Clear Commands
    //Gui::Command::clearCommands();
...
/*
    ...
    if (!sObjectName.isEmpty())
    {
        ...
        App::DocumentObject* pDocObject = pActiveDoc->getObject(sObjectName.toUtf8().data());

        if (pDocObject != 0)
        {
            PyObject* pDocObjectPy = pDocObject->getPyObject();
            ...
            if (pDocObjectPy != 0)
            {
                ...
                if (PyObject_HasAttrString(pDocObjectPy, "Proxy") == 1)
                {
                    //Get Proxy Attribute
                    //PyObject* pProxy = PyObject_GetAttrString(pDocObjectPy, "Proxy");
                    ...
                    {
                        //Call process
                        PyObject_CallMethod(pProxy, "process", sData.toUtf8().data());
                    }
                //}

            }

            pActiveDoc->removeObject(sObjectName.toUtf8().data());
        }
    }*/
}
...
void BrowserView::load(const char* URL)
{
    ...
    load(url);
    view->ConnectWebFrames(view->page()->mainFrame());
}
...
QString BrowserView::getLastAlert() const
{
#ifndef QTWEBENGINE
    CWebPage *pWebPage = view->webPage();
    return pWebPage->lastAlert(); 
#else
    return QString();
#endif
}
...
QString BrowserView::runJavaScript(const char *cmd) const
{
#ifndef QTWEBENGINE
    //Main Frame
    QWebFrame *pMainFrame = view->page()->mainFrame();
    return runJavaScript(pMainFrame, cmd);
#else
    return QString();
#endif
}
...
QString BrowserView::runJavaScript(QWebFrame* pFrame, const char* cmd) const
{
    //Try this command on the main frame
    QVariant var = pFrame->evaluateJavaScript(QString::fromUtf8(cmd));
    QString sStr = var.toString();
    ...
    for (int i = 0; i < nChildCount; i++)
    {
        sStr = runJavaScript(pChildFrames[i], cmd);
        if (!sStr.isEmpty())
            return sStr;
    }
...
void BrowserView::RegisterSlotHandler(const QString sData)
{
    view->RegisterSlotHandler(sData);
}
