...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "/Users/sapiriscfdc/Costumers/Hermes/pipe/caseConfiguration/system/fvSchemes",
        ),
(
            "LargeRoom_2",
            "/Users/sapiriscfdc/Costumers/Hermes/LargeRoomSimpleFoam/LargeRoom_2.json",
            "/Users/sapiriscfdc/Costumers/Hermes/LargeRoomSimpleFoam/caseConfiguration/system/fvSchemes",
        ),
        (
            "Flow_2",
            "/Users/sapiriscfdc/Costumers/Hermes/EWTModel/Flow_2.json",
            "/Users/sapiriscfdc/Costumers/Hermes/EWTModel/caseConfiguration/system/fvSchemes",
        ),
    ],
)
def test_reverse_fvSchemes_against_inputs(tmp_path: Path, case_name: str, input_json_path: str, dict_path: str):
    """
    Compare reversed fvSchemes against the expected node from reference inputs.
    """
    ...
    if not dict_file_path.exists():
        pytest.skip(f"[{case_name}] fvSchemes not found at {dict_file_path}")

    with open(input_file_path, "r") as f:
        input_json = json.load(f)

    expected = input_json["workflow"]["nodes"]["fvSchemes"]

    # Temporary test directory
    system_dir = tmp_path / case_name / "system"
    system_dir.mkdir(parents=True, exist_ok=True)
    dst = system_dir / "fvSchemes"
    ...
    assert actual == expected, (
        f"[{case_name}] fvSchemes reverse result does not match expected.\n"
