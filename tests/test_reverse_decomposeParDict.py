...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "/Users/sapiriscfdc/Costumers/Hermes/pipe/caseConfiguration/system/decomposeParDict",
        ),
        (
            "LargeRoom_2",
            "/Users/sapiriscfdc/Costumers/Hermes/LargeRoomSimpleFoam/LargeRoom_2.json",
            "/Users/sapiriscfdc/Costumers/Hermes/LargeRoomSimpleFoam/caseConfiguration/system/decomposeParDict",
        ),
        (
            "Flow_1",
            "/Users/sapiriscfdc/Costumers/Hermes/EWTModel/Flow_2.json",
            "/Users/sapiriscfdc/Costumers/Hermes/EWTModel/caseConfiguration/system/decomposeParDict",
        ),

    ],
)
def test_reverse_decomposeParDict_against_inputs(tmp_path: Path, case_name: str, input_json_path: str, dict_path: str):
    """
    Compare reversed decomposeParDict against the expected node from reference inputs.
    """
    ...
    if not dict_file_path.exists():
        pytest.skip(f"[{case_name}] decomposeParDict not found at {dict_file_path}")

    with open(input_file_path, "r") as f:
        input_json = json.load(f)

    expected = input_json["workflow"]["nodes"]["decomposePar"]

    # Temporary test directory
    system_dir = tmp_path / case_name / "system"
    system_dir.mkdir(parents=True, exist_ok=True)
    dst = system_dir / "decomposeParDict"
    ...
    actual = {
        ...
        "version": 2,  # keep consistency
    }

    assert actual == expected, (
        f"[{case_name}] decomposeParDict reverse result does not match expected.\n"
